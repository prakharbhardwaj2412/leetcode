...
public:
    ...
    void bottomTraversal(Node* node, vector<int> &result) {
        if(isLeaf(node)) result.push_back(node->data);
        if(node->left) bottomTraversal(node->left, result);
        if(node->right) bottomTraversal(node->right, result);
    }
    void rightTraversal(Node* node, vector<int> &result) {
